...
class Sprite{
	...
	createFromInfo(spriteInfo,container,img){		
		...
		if(spriteInfo.talking != undefined){
			this.#talking.swith = true;
			this.#talking.layerTalk = spriteInfo.talking.talking;
			this.#talking.layerShut = spriteInfo.talking.shut;
			var talkingCheckbox = $("<div style='padding-left:30px'><button class='customToggle'>talk</button></div>").appendTo(".input_"+spriteInfo.name);		
			talkingCheckbox.find("button").on("click",function(e){
				self.toggleTalking();
			});
		}
	...
	animationUpdate(){
		var dom = this.#dom.find(".animated_sprite:not(.d-none)");
	...
	talk(isTalking){
		if(this.#talking.switch == false || this.#talking.actualState==isTalking) return;
		this.chooseLayer(isTalking?this.#talking.layerTalk:this.#talking.layerShut);
		this.#talking.actualState = isTalking;
	}
	...
	#wiggle = new Wiggle();
	#wiggleSwitch = false;
	#talking = {"switch":false,"layerTalk":"","layerShut":"","actualState":false};
	#animated = false;
	#timeCounter = 0;
	// End Property
	...
	setPosition(pos){
		this.#pos = pos;
		$(this.#dom).css({top: (this.#pos.y)+"px",left:(this.#pos.x)+"px"});
	}
	...
	toggleTalking(forced = undefined){
		this.#talking.switch = forced!=undefined?forced:!this.#talking.switch;
		if(!this.#talking.switch){
			chooseLayer(this.#talking.layerShut);
		}
	}
